...
class SolarSystem:
    ...
    def execute(self):
        while self.run:
            ...
            self.escape_handle(keys_pressed)
            self.planets_movement()
            self.draw_window()

    def zoom_handle(self, keys_pressed):
        if keys_pressed[pygame.K_UP] and (self.SCALE >= 0.5):
            self.SCALE -= 0.1
        if keys_pressed[pygame.K_DOWN] and (self.SCALE <= 9):
            self.SCALE += 0.1

    def escape_handle(self, keys_pressed):
    ...
    def planets_movement(self):
        for planet in self.PLANETS:
            ...
            # planet_rect.height = planet.height
            # planet_rect.width = planet.width
...
class Planet:
    def __init__(self, name, width, height, or_period, e, semimajor, perihelion, angle):
        ...
        if semimajor > 227939366:
            self.semimajor = 227939366 + semimajor/10
        if perihelion > 206650000:
            self.perihelion = 206650000 + perihelion/10
...
def main():
    SUN = Planet('sun', 50, 50, 0, 0, 0, 0, 0)
    MERCURY = Planet('mercury', 20, 20, 87.9691, 0.240846, 57.91e6, 46.00e6, 0)
    VENUS = Planet('venus', 29, 29, 583.92, 0.006772, 108.21e6, 107.48e6, 90)
    EARTH = Planet('earth', 30, 30, 365.256, 0.0167086, 149598023, 147098450, 0)
    MARS = Planet('mars', 24, 24, 686.980, 0.0934, 227939366, 206650000, 180)
    JUPYTER = Planet('jupyter', 70, 70, 4332.59, 0.0489, 778477399.54866, 740599218.68742, 270)
    SATURN = Planet('saturn', 112, 60, 10755.70, 0.0565, 1433.53e6, 1352.55e6, 0)
    URANUS = Planet('uranus', 40, 40, 30688.5, 0.04717, 2870971632.0501, 2735561623.4073, 90)
    NEPTUNE = Planet('neptune', 40, 40, 60195, 0.008678, 4498407971.949, 4459512525.567, 270)
